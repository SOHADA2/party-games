...
- **현재 버전**: v0.3.0 (`index.html`의 `APP_VERSION`)
...
| # | 게임 | `tool` | 앱이 해줄 일 | 폰 | 상태 |
|---|---|---|---|---|---|
| 3 | 몸으로/한글자로 말해요 | `deck` | 제시어 덱·타이머·패스·팀 점수판 | 진행자 | ✅ **완료(v0.3.0)** |
| 1 | 멕썸노이즈 | (예정 `deck`) | 문장 자동생성·타이머·술래 로테이션 | 진행자 | 규칙만 |
| 6 | 연기 대결 | (예정 `deck`) | 대사×감정 랜덤·탈락 진행·투표 | 진행자 | 규칙만 |
| 2 | 스마일~! | (예정) | 술래 지정·10장 카운터·라운드 관리 | 술래 | 규칙만 |
| 7 | 컵 레이스 | (예정) | 대진표·기록 타이머 | 진행자 | 규칙만 |
| 5 | 안 봤어도 들키지마 | (예정) | 출제자 순번·10분 타이머·지목 집계·코인 정산 | 진행자 | ⚠️ 규칙 미완 |
| 4 | 상황극 마피아 | (예정) | 비밀 역할 배정·시나리오·밤낮 진행 | **각자 폰** | 규칙만 |

**다음 순서 = 1(멕썸노이즈) → 6(연기 대결).** 둘 다 `deck` 틀을 그대로 재사용한다.
